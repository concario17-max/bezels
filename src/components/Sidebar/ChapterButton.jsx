...
const ChapterButton = ({ chapter, count, isExpanded, onClick, isSubchapter = false, visibleHeadings = null }) => {
    ...
    return (
        <motion.div
            ...
            className={`w-full ${isSubchapter ? 'pl-3.5' : 'pl-2'}`}
        >
            {hasHeadings ? (
                <div className="mb-0.5 flex flex-col gap-0.5 px-2 pt-1">
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={onClick}
                className={`w-full flex items-start justify-between gap-1.5 px-2 py-1 rounded-xl text-left transition-all duration-300 ${isExpanded
            ...
            >
                <div className="min-w-0 flex-1 pr-1 flex flex-col gap-0">
                    <span className={`block whitespace-pre-line text-[12px] leading-[1.32] font-inter break-keep ${hasHeadings ? 'font-medium tracking-[0.005em]' : 'font-bold tracking-[-0.015em]'} ${isExpanded ? 'text-[#1C2B36] dark:text-gold-light' : ''
                        }`}>
                        {actionLabel}
                    </span>
                </div>
                <motion.span
                    ...
                    className="shrink-0 mt-0 min-w-[20px] text-center text-[#A68B5C] px-1 py-0 rounded text-[10px] font-bold"
                >
                    {count || 0}
                </motion.span>
            </motion.button>
